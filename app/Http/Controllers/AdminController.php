...
class AdminController extends Controller
{
    public function Admin_Dashboard_Page()
    {
        ...
        $productCount = products::all()->count();
            ...
            return view('adminpanel.index', compact('allShops', 'CountPendingRequests', 'customerCount', 'shopCount', 'productCount'));
        }
    }
    ...
    public function company_Table()
    {
        if (!session()->has('admin_id')) {
            ...
            $allCategories = categories::all();
            $allCompanies = DB::table('make')
                ->join('categories', 'categories.id', '=', 'make.catid')
                ->select('categories.name as catname', 'make.name as Companyname', 'make.id as makeid')
                ->get();
            return view('adminpanel.make', compact('allCompanies', 'allCategories'));
        }
    }
    ...
    public function Add_company(Request $request)
    {
        if (!session()->has('admin_id')) {
            ...
            $s = company::where('name', strtolower($request->input('company')))->count();
            if ($s == 0) {
                $s = new company();
                $s->name = strtolower($request->input('company'));
                $s->catid = $request->input('category');
                $s->save();
                return redirect()->back()->with('success', 'company Added Successfully');
            } else {
                return redirect()->back()->with('failure', 'company Already Exist ');
            }
        }
    }
    ...
    public function Confirm_Withdraw($id, $email)
    {
        if (!session()->has('admin_id')) {
            ...
            // Mail::to('info.shasar@gmail.com')->send(new sendcontactmail($details));
    ...
    public function Admin_company_Shop($id)
    {
        if (!session()->has('admin_id')) {
            return redirect('admin_login');
        } else {
